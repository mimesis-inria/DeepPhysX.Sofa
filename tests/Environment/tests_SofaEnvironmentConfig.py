...
class TestSofaEnvironmentConfig(TestCase):
    ...
    def test_create_environment(self):
        # ValueError
        with self.assertRaises(ValueError):
            SofaEnvironmentConfig(environment_class=Test1).create_environment(environment_manager=None)
        # TypeError
        with self.assertRaises(TypeError):
            SofaEnvironmentConfig(environment_class=Test2).create_environment(environment_manager=None)
        # No error
        environment_config = SofaEnvironmentConfig(environment_class=Env.TestEnvironment)
...
class Test2(Sofa.Core.Controller):
    def __init__(self, root_node, environment_manager, as_tcp_ip_client, *args, **kwargs):
